...
#     cursor = images_collection.find({"Date": {"$type": "string"}})
...
#             images_collection.update_one({"_id": document["_id"]}, {"$set": {"Date": date_datetime}})
#             print(f"Updated document ID {document['_id']} with date {date_datetime}")
#         except ValueError:
#             print(f"Skipping document ID {document['_id']} with invalid date format {date_string}")
